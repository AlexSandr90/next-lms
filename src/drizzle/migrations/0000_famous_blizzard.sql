...
CREATE TABLE "course_products" (
	...
	"productUd" uuid NOT NULL,
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "course_products_courseId_productUd_pk" PRIMARY KEY("courseId","productUd")
);
--> statement-breakpoint
...
ALTER TABLE "course_products" ADD CONSTRAINT "course_products_productUd_products_id_fk" FOREIGN KEY ("productUd") REFERENCES "public"."products"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
